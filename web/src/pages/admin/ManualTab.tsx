    ...
    {composing && !denied && <SitePanel className="max-w-2xl space-y-5 p-4 sm:p-6">
      ...
      <form id="admin-manual-form" className="space-y-5" noValidate onSubmit={event => {
        ...
        <fieldset disabled={submit.isPending || !!review} className="space-y-4">
          <fieldset><legend className="mb-2 text-sm font-medium text-clover-900">收款账号类型</legend><div className="grid gap-2 sm:grid-cols-2">{([['station', '福利站用户 ID'], ['newapi', 'new-api 用户 ID']] as const).map(([value, label]) => <button key={value} type="button" aria-pressed={namespace === value} className={cn('min-h-11 rounded-xl border px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-clover-500', namespace === value ? 'border-clover-solid-soft bg-clover-solid-soft text-white' : 'border-clover-200 bg-surface text-clover-800')} onClick={() => { setNamespace(value); setRecipientText('') }}>{label}</button>)}</div></fieldset>

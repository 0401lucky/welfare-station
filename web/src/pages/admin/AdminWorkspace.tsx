...
function AdminWorkspace({ user, GamePanel }: { user: User; GamePanel: ComponentType }) {
  ...
  return <>
    <nav aria-label="后台区块" className="-mx-4 mb-4 flex gap-2 overflow-x-auto px-4 pb-2 md:hidden">{sections.map(({ id, label, Icon }) => <Link key={id} to={href(id)} aria-current={tab === id ? 'page' : undefined} className={cn('flex min-h-11 shrink-0 items-center gap-1.5 rounded-full border px-3.5 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-clover-500', tab === id ? 'border-clover-solid-soft bg-clover-solid-soft font-semibold text-white' : 'border-clover-200 bg-surface/90 text-clover-800')}><Icon size={15} aria-hidden="true" />{label}</Link>)}</nav>
    <div className="flex items-start gap-5 lg:gap-6">
      <aside className="sticky top-36 hidden w-44 shrink-0 md:block lg:top-24"><nav aria-label="后台区块" className="space-y-1 rounded-2xl border border-clover-100 bg-surface/85 p-2">{sections.map(({ id, label, Icon }) => <Link key={id} to={href(id)} aria-current={tab === id ? 'page' : undefined} className={cn('flex min-h-11 items-center gap-2 rounded-xl border-l-2 px-3 py-2.5 text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-clover-500', tab === id ? 'border-clover-500 bg-clover-100/75 font-semibold text-clover-900' : 'border-transparent text-clover-700 hover:bg-clover-50')}><Icon size={17} aria-hidden="true" />{label}</Link>)}</nav></aside>

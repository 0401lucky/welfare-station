      ...
      <form id="admin-checkin-form" onSubmit={submit} noValidate>
        <SitePanel className="space-y-5 p-4 sm:p-6">
          ...
          <fieldset>
            ...
            <div className="flex flex-wrap gap-2">{([['permanent', '永久额度'], ['temporary', '限时额度']] as [QuotaType, string][]).map(([value, label]) => <button key={value} type="button" aria-pressed={(config.reward_type || 'permanent') === value} onClick={() => patch({ reward_type: value })} className={cn('min-h-11 rounded-full border px-4 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-clover-500', (config.reward_type || 'permanent') === value ? 'border-clover-600 bg-clover-600 text-white' : 'border-clover-200 bg-white text-clover-800')}>{label}</button>)}</div>

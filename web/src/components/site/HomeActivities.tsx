    ...
    <section className="site-anchor mt-7 sm:mt-8" id="welfare-activities" aria-labelledby="activities-heading">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-x-4 gap-y-3">
        ...
        <div className="flex max-w-full flex-wrap items-center gap-1.5" aria-label="活动分类">
          {filters.map(({ value, label }) => <button key={value} type="button" aria-pressed={filter === value} disabled={claim.isPending} onClick={() => setFilter(value)} className={cn('min-h-11 rounded-full px-3 text-xs transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-clover-500 disabled:opacity-60 sm:text-sm', filter === value ? 'bg-clover-700 font-medium text-white' : 'bg-white/60 text-clover-700 hover:bg-clover-100')}>{label}{query.data && <span className="ml-1 text-[11px] opacity-80">{query.data.filter((activity) => matchesHomeActivityFilter(activity.status, value)).length}</span>}</button>)}
                ...
                return <SitePanel key={activity.id} className="overflow-hidden p-5">
                  <div className="grid items-center gap-4 lg:grid-cols-[minmax(0,1.5fr)_minmax(0,.65fr)_minmax(0,1fr)_auto] lg:gap-5">
                    <div className="flex min-w-0 items-start gap-3">
                      <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-clover-100 bg-clover-50"><Clover size={25} stem={false} petal={active ? '#35a465' : '#8fd6a8'} petalAlt={active ? '#5bbc82' : '#bce3c9'} /></span>
